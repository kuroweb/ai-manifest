...
description: >
  references/taktサブモジュール更新時に、takt-*スキル群（takt-task-builder,
  takt-workflow-builder, takt-facet-builder, takt-analyzer,
  takt-optimizer）を最新のtaktバージョンに追従させるスキル。 TypeScriptスキーマ（taskRecordSchemas.ts /
  taskExecutionSchemas.ts）、ワークフローYAML、ファセットMarkdownの差分を検出し、
