...
description: >
  ...
  参照資料として活用する。ユーザーの要件をヒアリングし、movement構成、ルール設計、 ファセットファイル生成を一括で行う。
  トリガー：「ピースを作りたい」「ワークフローを定義」「taktのピースを作成」 「新しいtaktピースを作って」「takt
  piece」「ワークフローYAML」
---
# TAKT Piece Builder
...
| ビルトイン | 構成・用途 |
|-----------|------------|
| `default.yaml` | 標準開発（plan→write_tests→implement→ai_review→reviewers→fix→supervise） |
| `dual.yaml` / `dual-mini.yaml` / `dual-cqrs.yaml` / `dual-cqrs-mini.yaml` | フロント＋バック／CQRS／最小 |
| `frontend.yaml` / `frontend-mini.yaml` | フロントエンド特化（v0.32〜 plan は `plan-frontend` 出力契約を使用） |
| `backend.yaml` / `backend-mini.yaml` / `backend-cqrs.yaml` / `backend-cqrs-mini.yaml` | バックエンド／CQRS |
| `audit-architecture.yaml` と `-frontend`/`-backend`/`-dual` | アーキテクチャ監査（読み取り専用、コード変更なし） |
| `audit-e2e.yaml` / `audit-unit.yaml` / `audit-security.yaml` | E2E・ユニット・セキュリティ監査（読み取り専用）。旧 `fill-e2e`/`fill-unit` 廃止・`security-audit`→`audit-security` リネームに相当 |
| `review-default.yaml` / `review-fix-default.yaml` | 汎用レビュー→修正（旧 `review.yaml` / `review-fix.yaml`） |
| `review-dual.yaml` / `review-fix-dual.yaml` / `review-dual-cqrs.yaml` / `review-fix-dual-cqrs.yaml` | デュアル／CQRS 向けレビュー |
| `review-backend.yaml` / `review-fix-backend.yaml` / `review-backend-cqrs.yaml` / `review-fix-backend-cqrs.yaml` | バックエンド向け |
| `review-frontend.yaml` / `review-fix-frontend.yaml` | フロント向け |
| `takt-default.yaml` / `review-takt-default.yaml` / `review-fix-takt-default.yaml` | TAKT 自体の開発・レビュー用 |
| `research.yaml` / `deep-research.yaml` / `compound-eye.yaml` / `magi.yaml` / `terraform.yaml` | 調査・特殊ワークフロー |
...
| `pieceMcpServers` / `pieceArpeggio` / `pieceRuntimePrepare.customScripts` / `syncConflictResolver.autoApproveTools` | v0.33.0〜: ピース YAML の MCP・Arpeggio・runtime prepare のカスタムスクリプト・sync 競合解決の自動ツール承認は **デフォルト拒否**。使う場合はプロジェクト／グローバル設定で明示許可 |
