...
description: "会社目標に整合するチーム OKR を検討する。定性的な Objective と測定可能な Key Result を組み合わせた案を作る。四半期 OKR の策定、チーム目標と会社戦略の接続、Objective の草案作成、良い OKR の書き方整理に使う。"
---
...
7. **成果物を保存する**: 内容が十分大きければ `~/.docs/pm-exec/OKRs-[team-name]-[quarter].md` という名前の Markdown として保存する。
