...
- **生成物**: `rulesync generate` で `.cursor/`（上記に対応する `rules` / `skills` / `agents` など）と `.claude/` `.codex/` `.gemini/` が更新される
- **ホーム反映**: `bash scripts/install.sh` で `~/.cursor` などへシンボリックリンクを張る

hooks、MCP、Cursor CLI、Claude Code の個別設定などは rulesync の対象外。
...
`cli-config.json` は Cursor CLI が `~/.cursor/cli-config.json` を直接更新するため、`install.sh` ではシンボリックリンクを張らない。リポジトリ内のファイルは参考用で、必要なら手でコピーする。
