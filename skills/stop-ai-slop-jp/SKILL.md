...
description: >-
  AIで書いた日本語を、人間が書いた文章に戻す。
  ユーザーが `stop-ai-slop-jp` または `/stop-ai-slop-jp` で明示的に呼び出した場合にだけ使用すること。
  スキル名が無いときは使わない。
...
1. **命題型H2を避ける**。見出しは「テーマの名前」(名詞句)で立てる。「○○は△△だ」型は各節を小さなTED talkにする。
2. **二項対比を直接Yに変える**。「AではなくBだ」「単なるAではなくB」を「Bだ」と直接書く。
3. **ムラを入れる**。段落の長さ、トーン、結論の有無、密度、文末語尾を意図的にバラけさせる。全段落を同じ強度で着地させない。
4. **3項目並列を疑う**。「3つのポイント」「3つの観点」「3つの原則」を、2つか1つに削れないか考える。AIは3つ並べたがる。

### C. 語彙と記号

 1. **偏愛語・必殺技造語・副詞を撒かない**。「泥臭さ」「手触り」「解像度」「本質」「営み」「文脈」、および「真理」「虚飾」「禁欲的」「美学」「境地」のような必殺技造語を1記事に複数撒かない。「非常に」「めっちゃ」「普通に」「ガチで」は1段落に2個以上重ねない。禁止リストは `references/phrases.md`。
 2. **横文字メタファーを日本語に戻す**。「思考のOSをアップデート」「人生をハック」「習慣をインストール」「マインドをリファクタリング」を、「考え方を変える」「工夫する」「習慣をつける」「考え方を整理する」に戻す。「コンテキスト」「アライメント」「コミュニケーション」のような普通の日本語に戻せるカタカナ語も、戻す。
 3. **形容詞や心情に「」を使わない**。固有名詞か直接引用以外で「」を使わない。「軽い」「動けばヨシ」のような普通の語を引用符で囲まない。
 4. **記号のアーティファクトを潰す**。全角ダッシュ ── はコロン・改行・読点に置換。中黒並列 A・B・C は「と」「や」で繋ぐ。`**` 残骸、コロン後の半角スペース、装飾絵文字(🚀🎯✨💡)の均等撒布は検索 → 全置換。
