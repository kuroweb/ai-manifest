...
rules[6]:
  - path: @.gemini/memories/answer-style.md
    description: 回答は前置きを省き、結論ファーストで率直に伝える
    applyTo[1]: **/*
  - path: @.gemini/memories/avoiding-ambiguous-suffixes.md
    description: 曖昧なサフィックスを避ける：型・モジュール命名で責務と境界を明確にする
    applyTo[1]: **/*
  - path: @.gemini/memories/code-explanation.md
    description: コード説明は変更前後の差分と意図を説明する
    applyTo[1]: **/*
  - path: @.gemini/memories/explain-skill-selection.md
    description: スキル呼び出し前に選択スキルと理由を明示するルール
    applyTo[1]: **/*
  - path: @.gemini/memories/learning-before-coding.md
    description: コーディング前の学習：新しいコードを書く前に既存実装を分析する
    applyTo[1]: **/*
  - path: @.gemini/memories/less-is-more.md
