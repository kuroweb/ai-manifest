...
| TAKT 用 | `.takt/config.yaml`, `.takt/pieces/`, `.takt/facets/` | `config.yaml` に provider / model（`persona_providers`）と `provider_options`（`codex` / `claude` など）。ピース YAML はワークフロー定義。`bash scripts/install.sh` で `~/.takt/config.yaml` にもリンクされる。 |
