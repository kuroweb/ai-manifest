...
| TAKT 用 | `.takt/config.yaml`（プロジェクト）, `.takt_global/config.yaml`（グローバル正本）, `.takt/pieces/`, `.takt/facets/` | `.takt/` は TAKT がすべてプロジェクト設定として読む。グローバル専用は `.takt_global/config.yaml` に書き、`install.sh` が `~/.takt/config.yaml` へリンクする。 |
