---
name: agent
description: 'Cursor Agent CLI（`agent`）を使用してコードや文言について相談・レビューを行う。 トリガー: "agent", "cursor agent", "agentと相談", "agentに聞いて", "コードレビュー", "レビューして" 使用場面: (1) 文言・メッセージの検討、(2) コードレビュー、(3) 設計の相談、(4) バグ調査、(5) 解消困難な問題の調査'
---
# Cursor Agent（agent）

Cursor Agent CLI を使用してコードレビュー・分析を実行するスキル。方針は `codex` スキルと同じく、**編集せず読み取り・提案に寄せる**（`--plan`）。
...
4. 上記コマンド形式で agent を実行（要: Cursor Agent の認証）
5. 結果をユーザーに報告
