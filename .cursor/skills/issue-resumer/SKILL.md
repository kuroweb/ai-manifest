---
name: issue-resumer
description: '`/issue-resume` 実行時に `~/.docs/issues/` 配下（`close` を除く）から再開対象の issue を特定して読み込む。 `issue-creator` が作成した `~/.docs/issues/<state>/<slug>/issue.md` を再開導線として扱い、 複数候補がある場合は必ずユーザーに対象を確認する。 トリガー: `/issue-resume`, 「issue再開」, 「前回のissueの続き」, 「issueを読み込んで」。'
---
