---
name: issue-creator
description: 調査メモ、実装タスク、設計整理、引き継ぎ用の issue を `~/.docs/issues/draft/<slug>/issue.md` に作成するスキル。ユーザーが 「issues にメモして」「調査用の issue を作って」「issue を作成」など、 作業開始前に issue の下書きやメモの器を作りたい依頼をしたら使う。 目的・概要・対象プロジェクトを確認し、他の作業に入る前に issue.md を先に生成する。 既存 issue の再開は issue-resumer を使い、状態変更はサブコマンドで扱う。
---
# Issue Creator
